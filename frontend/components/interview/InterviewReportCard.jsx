...
/**
 * Full interview report — shared by the student's own
 * /mock-interview/report and the admin's read-only /admin/interviews/[id],
 * so a human admin sees exactly the same depth a student would, not a
 * separately-derived summary. `showActions` hides the student-specific
 * "practice weak skills / update skill gap" CTAs for the admin view.
 */
export default function InterviewReportCard({ session, showActions = true }) {
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-secondary">
              {answeredTurns.length === 0
                ...
                : `Only ${answeredTurns.length} question${answeredTurns.length === 1 ? "" : "s"} were answered.`}
            </p>
          </div>
        </div>
      )}
          ...
          {weakSkills.length > 0 && (
            <div className="rounded-2xl border border-outline-variant bg-white p-8 ambient-shadow print:hidden">
              <h3 className="mb-4 text-headline-md font-bold">
                {showActions ? "What to Practice Next" : "Skills to Improve"}
              </h3>
              <div className={showActions ? "mb-5 flex flex-wrap gap-2" : "flex flex-wrap gap-2"}>
