...
from app.services.cv_parser import extract_text, parse_cv_skills
...
class ResumeParseResult(BaseModel):
    ...
    note: str = "Review these skills, then save them via POST /api/profile."
...
async def upload_resume(file: UploadFile = File(...)) -> ResumeParseResult:
    """Extract skills from a PDF/DOCX resume via Groq.

    The parsed skills are returned for the student to review — nothing is
    saved until they confirm via POST /api/profile with skills_source='cv'.
    """
    ...
    parsed = await parse_cv_skills(text)
